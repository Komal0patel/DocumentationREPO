    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            if (extension.matches("\\.(jpg|jpeg|png|webp)")) {
                ...
                testResults = extractTestValues(pdfText);
            } else if (extension.matches("\\.(xlsx|xls)")) {
                testResults = extractFromExcel(filePath);
            } else if (extension.equals(".csv")) {
                testResults = extractFromCSV(filePath);
            } else {
                System.out.println("Unsupported file type.");
                return;
            }

            // Print final extracted results for all file types
            if (!testResults.isEmpty()) {
                System.out.println("\nFinal Extracted Test Results:");
                testResults.forEach((key, value) -> System.out.println(key + ": " + value));
            } else {
